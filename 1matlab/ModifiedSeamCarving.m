    ...
    for i = 1:1:rows
        for j = 1:1:cols
            if(i== 1)
                ...
                if(j == 1)
                    EnergyMap(i,j) = GradMean(i,j)+ min((EnergyMap(i-1,j) + costVertical(i,j,ReducedGrayImage)), (EnergyMap(i-1,j+1)+costRight(i,j,ReducedGrayImage)));
                elseif(j == cols)
                    EnergyMap(i,j) = GradMean(i,j)+ min((EnergyMap(i-1,j-1)+ costLeft(i,j,ReducedGrayImage)), (EnergyMap(i-1,j)+ costVertical(i,j,ReducedGrayImage)));
                else
                    EnergyMap(i,j) = GradMean(i,j)+ min(min((EnergyMap(i-1,j-1)+ costLeft(i,j,ReducedGrayImage)), (EnergyMap(i-1,j)+ costVertical(i,j,ReducedGrayImage))), (EnergyMap(i-1,j+1)+costRight(i,j,ReducedGrayImage)));
                end
